...
- If the P3 Google Sheets Guard is enabled, use only its dedicated synthetic-data Google account,
  connect the exact approved spreadsheet, and verify a denied observation produces no Google API
  read before testing the approved fixed range.
...
| P3 fixed-range synthetic Google Sheets reads | [`packages/google-sheets-guard`](packages/google-sheets-guard/README.md) | Yes |
